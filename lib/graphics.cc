...
int DrawText(Canvas *c, const Font &font,
             ...
             const char *utf8_text, int width_offset, int width, bool center) {
  return DrawText(c, font, x, y, color, NULL, utf8_text, width_offset, width, center);
}

int DrawText(Canvas *c, const Font &font,
             int x, int y, const Color &color, const Color *background_color,
             const char *utf8_text, int extra_spacing, int width_offset, int width, int initial_offset, bool center) {
  //const int start_x = x;

  const char* test_text = utf8_text;
  int char_num = 0;
  while(*test_text) {
    utf8_next_codepoint(test_text);
    char_num++;
  }

  if (initial_offset > 0) {
        if(initial_offset > width_offset){
            initial_offset -= width_offset;
        } else {
            width_offset -= initial_offset;
            initial_offset = 0;
        }
    }
  ...
  //round down to nearest character width
  int char_to_skip = width_offset / font.CharacterWidth('W');
  int char_offset = width_offset % font.CharacterWidth('W');
  //detrmine the the number of characters in utf8_text
  //int num_items = sizeof(utf8_text) / sizeof(const char);
  int total_width = font.CharacterWidth('W') * char_num + extra_spacing * char_num;
  int center_offset = (width - total_width) / 2;
  //cout << "num_items: " << num_items << endl;
//   cout << "total_width: " << total_width << endl;
//   cout << "width: " << width << endl;
//   cout << "center_offset: " << center_offset << endl;
  if (center_offset > 0 && center) {
    x += center_offset;
    char_to_skip = 0;
    char_offset = 0;
  }
  //int total_offset = char_to_skip * font.CharacterWidth('W') + char_offset;

  char_num = 0;
  while (*utf8_text) {
    if (char_num < char_to_skip) {
      utf8_next_codepoint(utf8_text);
    } else if (char_num == char_to_skip) {
        const uint32_t cp = utf8_next_codepoint(utf8_text);
        x += font.DrawGlyph(c, x, y, color, background_color, cp, char_offset);
        x += extra_spacing;
    } else {
        const uint32_t cp = utf8_next_codepoint(utf8_text);
        x += font.DrawGlyph(c, x, y, color, background_color, cp, 0);
        x += extra_spacing;
    }
    char_num++;
  }
  return font.CharacterWidth('W') * char_num + extra_spacing * char_num;
}
...
int DrawText(Canvas *c, const Font &font,
             int x, int y, const Color &color, const Color *background_color,
             const char *utf8_text, int width_offset, int width, int initial_offset, bool center) {
  return DrawText(c, font, x, y, color, background_color, utf8_text, 0, width_offset, width, center);
}
...
int VerticalDrawText(Canvas *c, const Font &font, int x, int y,
  ...
  while (*utf8_text) {
    const uint32_t cp = utf8_next_codepoint(utf8_text);
    font.DrawGlyph(c, x, y, color, background_color, cp, 0);
