...
int Font::DrawGlyph(Canvas *c, int x_pos, int y_pos,
                    ...
                    uint32_t unicode_codepoint, int width_offset) const {
  const Glyph *g = FindGlyph(unicode_codepoint);
  ...
  for (int y = 0; y < g->height; ++y) {
    ...
    rowbitmap_t x_mask = (1LL<<(63 - width_offset));
  ...
  return g->device_width - width_offset;
}

int Font::DrawGlyph(Canvas *c, int x_pos, int y_pos, const Color &color,
                    uint32_t unicode_codepoint, int width_offset) const {
  return DrawGlyph(c, x_pos, y_pos, color, NULL, unicode_codepoint, width_offset);
}
